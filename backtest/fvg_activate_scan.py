...
print("="*60);print("  激活倍数扫描 — 1.0x ~ 3.5x");print("="*60)
...
def bt(act):
    ...
    for i in range(1,len(df1)):
        ...
        if po!=0:
            ...
            sl=ep-(av*SL_M)if po==1 else ep+(av*SL_M)
            act_p=ep+(av*act)if po==1 else ep-(av*act)
            if(po==1 and p>=act_p)or(po==-1 and p<=act_p):active=True
    ...
    return {"n":n,"wr":w/n*100,"pf":gw/gl if gl else 99,"rt":rt,"md":md,"kr":abs(rt/md)if md else 0,"act":active}

print("\n回测...")
res={}
for act in [1.0,1.5,2.0,2.5,3.0,3.5]:
    r=bt(act)
    if r:res[act]=r

print(f"\n{'='*65}")
print(f"{'激活x':>8}{'交易':>5}{'胜率':>7}{'PF':>6}{'收益':>10}{'回撤':>7}{'卡玛':>6}")
print("-"*65)
for act in sorted(res.keys()):
    r=res[act]
    print(f"  {act:.1f}x {r['n']:>4}{r['wr']:>6.1f}%{r['pf']:>6.2f}{r['rt']:>+9.2f}%{r['md']:>6.2f}%{r['kr']:>6.2f}")
